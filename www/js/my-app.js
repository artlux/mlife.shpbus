...
window.debug = false;
window.startUrl = 'http://bus.mlife-media.by/ajax/mobil_app/';
...
if(typeof(window.openDatabase) !== 'undefined'){
	db = window.openDatabase('shbus.db','1.0','shbus',1*1024*1024);
}else{
	db = window.sqlitePlugin.openDatabase({name: 'shbus',location: 'default'},function(db){});
}
